--[[
    PlayerShootGroundState
]]

PlayerShootGroundState = Class{__includes = BaseState}

function PlayerShootGroundState:enter(player)
    self.player = player
    ...
    self.shootAnimation = Animation(gPlayerAnimations['shoot_ground'])
    self.player.animation = self.shootAnimation
    self.player.animation:refresh()
end

function PlayerShootGroundState:update(dt)
    if self.player.direction == 'left' then
        ...
        if self.player.animation.currentFrame == 9 and not self.player.launched then
...
function PlayerShootGroundState:exit()
