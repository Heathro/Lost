...
PlayerFlipState = Class{__includes = BaseState}

function PlayerFlipState:enter(player)
    self.player = player

    self.player.offsetX = self.player.direction == 'left' and -2 or 2
    self.player.up = true

    self.player.texture = gTextures['flip']
    self.flipAnimation = Animation(gPlayerAnimations['flip'])
    self.player.animation = self.flipAnimation
...
function PlayerFlipState:update(dt)

    if self.player.direction == 'left' then
        self.player.offsetX = -2
    else
        self.player.offsetX = 2
    end
