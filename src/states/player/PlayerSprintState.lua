...
PlayerSprintState = Class{__includes = BaseState}

function PlayerSprintState:enter(player)
    self.player = player

    self.player.offsetX = self.player.direction == 'left' and -6 or 6

    self.player.texture = gTextures['sprint']
    self.sprintAnimation = Animation(gPlayerAnimations['sprint'])
    self.player.animation = self.sprintAnimation
...
function PlayerSprintState:update(dt)
    ...
    if love.keyboard.isDown(SPRINT) then
        ...
        elseif love.keyboard.wasPressed(JUMP_FLIP_GETUP) then
            ...
            self.player.sideVelocity = self.player.sprintSpeed
        ...
        elseif love.keyboard.isDown(LEFT) then
            ...
            self.player.dx = -self.player.sprintSpeed
...
function PlayerSprintState:exit()
